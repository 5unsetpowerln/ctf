...
struct GameState {
  ...
  unsigned short is_push[CHR_NUM]; // you can push this object if you move into
                                   // a cell with the object
  unsigned short
      is_stop[CHR_NUM]; // you cannot move into a cell with this object
  unsigned short is_you[CHR_NUM]; // you can controll this object with WASD keys
  unsigned short is_sink[CHR_NUM]; // all objects in a cell are destroyed when
                                   // something come onto a cell with the object
  unsigned short is_open[CHR_NUM]; // when *open* and *shut* objects are in the
                                   // same cell, both are destroyed
  unsigned short is_shut[CHR_NUM]; // when *open* and *shut* objects are in the
                                   // same cell, both are destroyed
  unsigned short
      is_win[CHR_NUM]; // you will win if *you* enter a cell with the object
  unsigned char should_update[STAGE_H][STAGE_W];
} state;

// object_charsからのオフセットを返す
int get_object_id(char obj) { return strchr(object_chars, obj) - object_chars; }
  ...
  for (int i = 0; i < STAGE_H; i++) {
    for (int j = 0; j < STAGE_W; j++) {
      if (state.stage[i][j] == 0) {
        ...
        // state.stageはshort型の二次元配列である。short型は2バイトすなわち16ビットなので、
        // state.stage[i][j]が0でない限り__builtin_ctzの結果はは15以下である。
        // また、0の場合はこの関数は実行されないのでここは脆弱ではない。
...
void move(unsigned short *from, unsigned short *to, unsigned short set) {
  if (set == 0)
    return;
  // 動かす前はfromにsetがいるので~setと&することでfromを0にできる
  *from &= ~set;
  // toがis_sinkである場合、もともとあったis_sinkなオブジェクトとsetが消滅するのでtoは0になる
  if (*to & to_bitset(state.is_sink)) {
    *to = 0;
  } else {
    *to |= set;
  }
  // 新しいtoがis_openかつis_shutである場合、toからis_openとis_shutの特性を消す
...
void step_game(int dx, int dy) {
  ...
  for (int y = 0; y < STAGE_H; y++) {
    for (int x = 0; x < STAGE_W; x++) {
      ...
      if (you == 0 || !state.should_update[y][x])
        continue;
      will_lose = 0;

      int nx = x + dx, ny = y + dy; // next position
      if (state.stage[ny][nx] & to_bitset(state.is_stop))
        continue;
      // 勝利条件はstate.stage[ny][nx]がstate.is_win内の文字であること
      // 雑にstate.stageをis_winな文字でたくさん上書きしても勝てる
      if (state.stage[ny][nx] & to_bitset(state.is_win))
        win();
  ...
  // is_youかつis_stopな場合にloseにする機構
  // おそらく壁抜け防止機構
  for (int y = 0; y < STAGE_H; y++) {
    for (int x = 0; x < STAGE_W; x++) {
      if (state.stage[y][x] & to_bitset(state.is_you) &&
          state.stage[y][x] & to_bitset(state.is_stop))
        lose();
    }
  }
}
...
int main(void) {
  ...
  // 64文字読み取った上で.yを最後にくっつけているから最終てkには66文字読み取ることになる
  // しかし、stage.stage_nameは長さが64文字なのでBOFが発生する
  // state.stage_nameの次はstate.spawn_offである。spawn_offはすでに初期化済みなので上書きできる
