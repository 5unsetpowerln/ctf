...
class Log {
  ...
  public:
	...
	void add_cmd_to_log(const char *cmd) { strcat(logs, cmd); }
...
class Variable {
  public:
	...
	Variable(long l) : type(LONG) { value.l = l; }

	Variable(const char *s) : type(STRING) { value.s = strdup(s); }

	virtual void print() { std::cout << "Default print" << std::endl; }
};

class longVariable : public Variable {
  public:
	longVariable(long l) : Variable(l) {}
	void print() override { std::cout << value.l << std::endl; }
};

class stringVariable : public Variable {
  public:
	stringVariable(const char *s) : Variable(s) {}
	void print() override { std::cout << value.s << std::endl; }
};
...
long getLongVar(const char *name) {
	...
	if (v->type == LONG) {
		...
		std::cout << "Invalid variable " << name << ": " << v->value.s
				  << std::endl;
		return 0;
	}
}
